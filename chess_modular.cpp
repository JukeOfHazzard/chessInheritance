 ...
 ***************************************************/
inline bool isNotWhite_modular(const char* board, int row, int col)
{
 ...
 ***************************************************/
inline bool isWhite_modular(const char* board, int row, int col)
{
   // not white if we are off the board or if we are looking at a space
   if (row < 0 || row >= 8 || col < 0 || col >= 8)
      return false;
 ...
 ***************************************************/
inline bool isNotBlack_modular(const char* board, int row, int col)
{
   // not white if we are off the board or if we are looking at a space
   if (row < 0 || row >= 8 || col < 0 || col >= 8)
      return false;
 ...
 ***************************************************/
inline bool isBlack_modular(const char* board, int row, int col)
{
   // not white if we are off the board or if we are looking at a space
   if (row < 0 || row >= 8 || col < 0 || col >= 8)
      return false;
 ...
 *********************************************************/
set <int> getPossibleMoves_modular(const char* board, int location)
{
   set <int> possible;
   ...
   bool amBlack = isBlack_modular(board, row, col);
   ...
   if (board[location] == 'N' || board[location] == 'n')
   {
      ...
      for (int i = 0; i < 8; i++)
      {
         r = row + moves[i].row;
         c = col + moves[i].col;
         if ( amBlack && isNotBlack_modular(board, r, c))
            possible.insert(r * 8 + c);
         if (!amBlack && isNotWhite_modular(board, r, c))
            possible.insert(r * 8 + c);
      }
   }
 ...
 ***************************************************/
void draw_modular(const char* board, const Interface & ui, const set <int> & possible)
{
   ogstream gout;
 ...
 *********************************************/
bool move_modular(char* board, int positionFrom, int positionTo)
{
   ...
   set <int> possiblePrevious = getPossibleMoves_modular(board, positionFrom);
 ...
 **************************************/
void callBack_modular(Interface *pUI, void * p)
{
   set <int> possible;
   ...
   // move
   if (move_modular(board, pUI->getPreviousPosition(), pUI->getSelectPosition()))
      pUI->clearSelectPosition();
   else
      possible = getPossibleMoves_modular(board, pUI->getSelectPosition());
   ...
   // draw the board
   draw_modular(board, *pUI, possible);
 ...
 *******************************************************/
void parse_modular(const string& textMove, int& positionFrom, int& positionTo)
{
 ...
 *******************************************************/
void readFile_modular(const char* fileName, char* board)
{
   // open the file
   ...
   while (valid && fin >> textMove)
   {
      int positionFrom;
      int positionTo;
      parse_modular(textMove, positionFrom, positionTo);
      valid = move_modular(board, positionFrom, positionTo);
   }
...
int WINAPI WinMain(
   ...
   _In_ int nCmdShow)
#else // !_WIN32
int main_modular(int argc, char** argv) // not main because this file is essentially an archive
#endif // !_WIN32
{
   Interface ui("Chess");
 ...
 //  string filename = argv[1];
   if (__argc == 2)
      readFile_modular(__argv[1], board);
#else // !_WIN32
   if (argc == 2)
      readFile_modular(argv[1], board);
#endif // !_WIN32
